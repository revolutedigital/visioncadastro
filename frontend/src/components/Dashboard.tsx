...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const loadDataQuality = async () => {
    try {
      const response = await authFetch(`${API_BASE_URL}/api/data-quality/report`);
      const data = await response.json();
  ...
  const handleRecalculateScores = async () => {
    ...
    try {
      const response = await authFetch(`${API_BASE_URL}/api/analysis/recalculate-scores`, {
        method: 'POST',
      });
      const data = await response.json();
  ...
  const handleStartGeocoding = async () => {
    try {
      await authFetch(`${API_BASE_URL}/api/geocoding/start`, { method: 'POST' });
  ...
  const handleStartPlaces = async () => {
    try {
      await authFetch(`${API_BASE_URL}/api/places/start`, { method: 'POST' });
  ...
  const handleStartAnalysis = async () => {
    try {
      const response = await authFetch(`${API_BASE_URL}/api/analysis/start`, {
        method: 'POST',
  ...
  const handleStartEnrichment = async () => {
    try {
      const response = await authFetch(`${API_BASE_URL}/api/enrichment/start`, {
        method: 'POST',
  ...
  const handleCalcularTipologias = async () => {
    ...
    try {
      const response = await authFetch(`${API_BASE_URL}/api/tipologia/recalcular-todos`, {
        method: 'POST',
      });
      const data = await response.json();
